...
parser.add_argument('--truth-v-filename', type=str, required=True)
parser.add_argument('--mask-name', type=str, required=True)
...
vel = np.load(args.truth_v_filename)[..., [2, 1]]
mask_all = np.load(f'{containing_dir}/mat_results/{args.mask_name}').astype(bool)
# num_points = mask_all.shape[1] * mask_all.shape[2]
...
def render_field(containing_dir, frame_prefix, frame_id_30, pix_x, pix_y, cmap):
    ...
    mask = np.copy(mask_all[frame_id])
    mag = np.hypot(u, v)
    q = img_ax.quiver(pix_x[mask].ravel(),
                      pix_y[mask].ravel(),
                      u[mask.ravel()],
                      v[mask.ravel()],
                      mag[mask.ravel()],
                      ...
                      scale=5)
    fig.savefig(
        f'{containing_dir}/gridfield/frame30fps-offset-{frame_id_30}.png',
        dpi=my_dpi)
    plt.close('all')
