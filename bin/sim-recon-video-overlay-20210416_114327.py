...
def render_field_bc5d5a07(recon_dir, frame_id, score_filtered):
    fps = 30
    t = frame_id / fps
    ...
    ax_score.annotate("Circular",
                      ...
                      bbox=dict(boxstyle="square,pad=0.3",
                                ...
                                lw=1.5,
                                alpha=0.5))
    ax_score.xaxis.set_ticks(np.arange(0, 14.5, 1))
    ax_score.yaxis.set_ticks(np.arange(-1, 0.05, 0.2))
...
for frame_id in range(602):
    render_field_bc5d5a07(selected_hc_dir, frame_id, score_filtered)
