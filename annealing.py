...
solver.particle_x.read_file(f'.alcache/{read_prefix}-{num_particles}.alu')
...
radius_decrease_rate = 0
best_density = 0.0
best_radius = 0.0
best_density_diff = 1.0
best_x = dp.create((num_particles), 3)

# previous_stat =  np.load(f'.alcache/{read_prefix}-stat{num_particles}.npy')
pellet_filename = f'.alcache/{read_prefix}-{num_particles}-pellets.alu'
stage = 0
current_radius = np.load(f'.alcache/{read_prefix}-stat{num_particles}.npy')[0]
...
while True:
    ...
    if (mean_density <= 1 and best_density <= mean_density):
        ...
        if best_density < mean_density or density_diff < best_density_diff:
            ...
            best_x.write_file(
                f".alcache/annealing-best-{solver.num_particles}.alu",
                num_particles)
            fill_solver.particle_x.write_file(
                f".alcache/annealing-best-{solver.num_particles}-pellets.alu",
                num_pellets)
    dp.unmap_graphical_pointers()
    if (mean_density > 1):
        break
