...
while True:
    ...
    fill_pile.replace(0,
                      dp.InfiniteTubeDistance.create(
    ...
    fill_mode = 2
    fill_particle_radius = particle_radius * 0.6
    if not pellet_initialized:
        num_sample_positions = dp.Runner.get_fluid_cylinder_num_particles(
            outer_radius, -pipe_length_half, pipe_length_half, fill_particle_radius)
        ...
        runner.launch_create_fluid_cylinder_internal(x, internal_encoded, num_pellets,
                                                  0, outer_radius, fill_particle_radius, -pipe_length_half, pipe_length_half)
        dp.map_graphical_pointers()
    ...
    print('pellet_density', min_pellet_density, mean_pellet_density, max_pellet_density)
    ...
    for fill_step_id in range(100):
        display_proxy.draw()
        dp.map_graphical_pointers()
        for substep_id in range(100):
            # print('solve fill')
    ...
    best_x.write_file(f".alcache/latest-{solver.num_particles}.alu", solver.num_particles)
    fill_solver.particle_x.write_file(
        f".alcache/latest-{solver.num_particles}-pellets.alu", num_pellets)
    dp.unmap_graphical_pointers()
    if (mean_density > 1):
        break
    print("================")

best_x.write_file(f".alcache/best-{solver.num_particles}.alu", solver.num_particles)
fill_solver.particle_x.write_file(
    f".alcache/best-{solver.num_particles}-pellets.alu", num_pellets)
np.save(f".alcache/best-stat{solver.num_particles}",
